  ...
  do {
    ...
    do {
      ...
      for (attempt; attempt > 0; attempt--) {
        ...
        prizeForStroke = Math.floor(
          prizeMax / Math.pow(2, maxAttempts - attempt)
        ); 
        userNum = parseInt(
          prompt(
        ...
        if (userNum === secretNum) {
          ...
          prizeTotal = prizeTotal + prizeForStroke; 
          prizeMax = prizeMax * prizeCof; 
          maxRange = maxRange * rangeCof; 
          break;
        }
      }
      if (attempt === 0) {
      ...
      confirm(
        `Congratulation! Your prize is: ${prizeTotal}$ Do you want to continue?`
      )
    );
